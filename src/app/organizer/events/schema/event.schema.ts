...
export const eventSchema = z
    .object({
        ...
        start_time: z.string().min(1, "Start datetime is required"),
        thumbnail: z
            .string()
            .url("URL is invalid")
            .optional()
            .or(z.literal("").transform(() => undefined)),
        end_time: z.string().min(1, "End datetime is required"),
        category: z.enum([
            "technical",
            "business",
            "cultural",
    ...
    .superRefine((data, ctx) => {
        // start < end
        const s = Date.parse(data.start_time);
        const e = Date.parse(data.end_time);
