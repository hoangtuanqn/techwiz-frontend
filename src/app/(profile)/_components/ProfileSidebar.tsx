...
const links = [
    { href: "/profile", label: "Personal Information", icon: User, exact: true }, // Add exact prop
...
export default function ProfileSidebar() {
    ...
    return (
        <aside className="w-64 border-r border-slate-200 bg-white p-4"> {/* Updated styling */}
            {/* Logo */}
            <div className="mb-6 flex items-center gap-2">
                <CircleUserRound className="h-6 w-6 text-[#06b6d4]" />
                <span className="font-semibold">Profile</span> {/* Changed title */}
            </div>
            <nav className="flex flex-col space-y-1"> {/* Updated spacing */}
                {links.map((link) => (
                    <SidebarLink
                        key={link.href}
                        href={link.href}
