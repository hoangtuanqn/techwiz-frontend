...
import { eventSchema } from "~/app/organizer/events/schema/event.schema";
import { EventDetailResponseType } from "~/types/schemaZod/event.schema";
import { useUnsavedChangesWarning } from "~/hooks/useUnsavedChangesWarning";
type EventFormValues = z.infer<typeof eventSchema>;

export default function EventFormEdit({ event }: { event: EventDetailResponseType["data"] }) {
    const form = useForm<EventFormValues>({
        ...
        defaultValues: {
            ...
            capacity: event.seating.total_seats || 0,
            mode: event.mode ?? "",
            venue: event.venue ?? "",
            note: event.note ?? "",
    ...
    const eventMutation = useMutation({
        mutationFn: async (data: EventFormValues) => {
            ...
            return await eventApi.updateEvent(event.id, data);
        },
