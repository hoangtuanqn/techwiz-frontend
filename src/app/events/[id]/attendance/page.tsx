...
function ParticipantQR({ eventId }: { eventId: string }) {
    ...
    return (
        <section className="mt-8 rounded-2xl border border-slate-200 p-6">
            ...
            <p className="mt-1 text-sm text-slate-600">Show this QR to the organizer at the entrance to check in.</p>
        ...
        <section className="mt-8 rounded-2xl border border-slate-200 p-6">
            ...
            {supported === false && (
                <div className="mt-4 rounded-md border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800">
                    Your browser doesn’t support <code>BarcodeDetector</code>. Use the latest Chrome/Edge/Safari on
                    mobile/desktop, or implement a fallback using a QR library like <code>jsQR</code>.
                </div>
            )}
            ...
            {result && (
                <div className="mt-4 rounded-md border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-800">
                    ...
                    <pre className="mt-2 overflow-auto rounded bg-white/60 p-2 text-xs text-emerald-900">{result}</pre>
...
function ManualEntry({ eventId }: { eventId: string }) {
    ...
    return (
        <div className="mt-6 rounded-xl border border-slate-200 p-4">
            ...
            <p className="mt-1 text-xs text-slate-500">If the QR cannot be scanned, paste the token string here.</p>
