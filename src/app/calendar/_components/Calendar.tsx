...
const Calendar: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const numDays = weeks * 7; // 1 week = 7, 2 weeks = 14
  ...
  const rangeLabel = useMemo(() => {
    ...
    return sameMonth
      ...
      : `${s.toLocaleString("en-US", { month: "short" })} ${s.getDate()}, ${s.getFullYear()} — ${e.toLocaleString(
          "en-US",
          { month: "short" }
        )} ${e.getDate()}, ${e.getFullYear()}`;
  }, [days]);

  const navDisabled = weeks === 2; // lock nav in 2-week fixed mode

  // 👉 Index cột "hôm nay" trong mảng days
  ...
  return (
    <div className="bg-gradient-to-b from-slate-50 to-white text-slate-800 antialiased">
      <main className="mx-auto max-w-7xl px-4 pt-8 sm:px-6 lg:px-8">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {searchQuery && (
                <span className="inline-flex items-center gap-1 rounded-lg bg-cyan-100 px-2 py-1 text-xs font-medium text-cyan-700">
                  ...
                  <button onClick={() => setSearchQuery("")} className="ml-1 rounded-full p-0.5 hover:bg-cyan-200">
                    <X className="h-3 w-3" />
                  </button>
                </span>
              )}
              {selectedCategory !== "all" && (
                <span className="inline-flex items-center gap-1 rounded-lg bg-blue-100 px-2 py-1 text-xs font-medium text-blue-700">
                  {selectedCategory}
                  <button onClick={() => setSelectedCategory("all")} className="ml-1 rounded-full p-0.5 hover:bg-blue-200">
                    <X className="h-3 w-3" />
                  </button>
                </span>
              )}
              {selectedStatus !== "all" && (
                <span className="inline-flex items-center gap-1 rounded-lg bg-green-100 px-2 py-1 text-xs font-medium text-green-700">
                  {selectedStatus === "upcoming" ? "Upcoming" : selectedStatus === "ongoing" ? "Ongoing" : "Completed"}
                  <button onClick={() => setSelectedStatus("all")} className="ml-1 rounded-full p-0.5 hover:bg-green-200">
                    <X className="h-3 w-3" />
                  </button>
                </span>
              )}
              {showMyEvents && (
                <span className="inline-flex items-center gap-1 rounded-lg bg-purple-100 px-2 py-1 text-xs font-medium text-purple-700">
                  ...
                  My Events
                  <button onClick={() => setShowMyEvents(false)} className="ml-1 rounded-full p-0.5 hover:bg-purple-200">
                    <X className="h-3 w-3" />
                  </button>
                </span>
              )}
          ...
          <div className="flex items-center justify-between rounded-xl border border-cyan-100 bg-gradient-to-r from-cyan-50 to-blue-50 p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-slate-600">{apiLoading ? "Loading..." : `In the period ${rangeLabel}`}</p>
              </div>
            </div>
            {visibleEvents.length > 0 && (
              <div className="text-right">
                <div className="text-sm text-slate-600">
                  Registered: {visibleEvents.filter((ev: any) => ev.is_booked).length} events
                </div>
              </div>
            )}
          </div>
        ...
        {/* ======= Calendar (with Today column highlight overlay) ======= */}
        <div
          className="relative mb-5 overflow-x-auto overflow-y-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"
          data-aos="fade-up"
        >
          {/* Overlay highlight for today's column */}
          {todayIndex >= 0 && (
            <div className="pointer-events-none absolute inset-0 z-[5]">
              ...
              >
                <div className="relative" style={{ gridColumn: `${todayIndex + 1} / ${todayIndex + 2}` }}>
                  <div className="absolute inset-0 bg-cyan-300/10" />
                  <div className="absolute inset-0 bg-cyan-400/15 blur-2xl" />
                  <div className="absolute inset-y-0 left-0 w-px bg-cyan-300/40" />
                  <div className="absolute inset-y-0 right-0 w-px bg-cyan-300/40" />
                </div>
              </div>
            </div>
          )}

          {/* Header Days (sticky, above overlay) */}
          <div
          ...
          {/* Event Rows (overlay is above but pointer-events-none, so clicks still work) */}
          <div className="relative z-0 space-y-2 p-3">
            {apiLoading && (
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Link href={`/events/${ev.id}`} className="flex min-w-0 items-center gap-3">
                        <div className="min-w-0 flex-1">
                          <div className="truncate text-sm font-semibold tracking-tight">{ev.title}</div>
                          <div className={`text-[11px] ${faded ? "text-slate-600/90" : "text-white/90"}`}>
                            {startDate}
                      ...
                      <div className="ml-auto flex items-center gap-2">
                        {ev.is_booked && (
                          <span className="rounded-full bg-white/20 px-2 py-1 text-xs">Registered</span>
                        )}
                        {headerStatusChip(
                          statusChip(status),
